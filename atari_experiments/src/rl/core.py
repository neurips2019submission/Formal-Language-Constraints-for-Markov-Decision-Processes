...
from rl.callbacks import (CallbackList, TestLogger, TrainEpisodeLogger,
                          TrainIntervalLogger, Visualizer)
...
class Agent(object):
    ...
    def fit(self,
        ...
        try:
            while self.step < nb_steps:
                ...
                if self.processor is not None:
                    action = self.processor.process_action(action, q_values)
                reward = np.float32(0)
                accumulated_info = {}
                done = False
    ...
    def test(self,
             ...
             nb_max_start_steps=0,
             start_step_policy=None,
             verbose=1):  #, orig_dqn=None, log=None):
        ...
        for episode in range(nb_episodes):
            ...
            for _ in range(nb_random_start_steps):
                if start_step_policy is None:
                    ...
                    action = start_step_policy(observation)
                if self.processor is not None:
                    action = self.processor.process_action(action, None)
                callbacks.on_action_begin(action)
                observation, r, done, info = env.step(action)
                observation = deepcopy(observation)
                #orig_observation = deepcopy(observation)

                if self.processor is not None:
                    observation, r, done, info = self.processor.process_step(
                        observation, r, done, info)
                    self.processor.process_experience(observation, action, r,
                                                      done)

                #if orig_dqn.processor is not None:
                #    orig_observation, _, _, _ = orig_dqn.processor.process_step(orig_observation, r, done, info)
            ...
            while not done:
                ...
                action, q_values = self.forward(observation)
                #action_orig, q_values_orig = orig_dqn.forward(orig_observation)
...
class Processor(object):
    ...
    def process_experience(self, observation, action, reward, terminal):
