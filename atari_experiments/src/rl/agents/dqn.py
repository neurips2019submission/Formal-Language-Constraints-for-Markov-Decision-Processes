...
class DQNAgent(AbstractDQNAgent):
    ...
    def __init__(self, model, policy=None, test_policy=None, enable_double_dqn=True, enable_dueling_network=False,
                 dueling_type='avg', dfa_governor=False, *args, **kwargs):
...
class NAFAgent(AbstractDQNAgent):
    ...
    def backward(self, reward, terminal):
        # Store most recent experience in memory.
        if self.step % self.memory_interval == 0: 
            _obs, _recent_action, _reward, _terminal = self.processor.process_experience(self.recent_observation, self.recent_action, reward, terminal)
            self.memory.append(_obs, _recent_action, _reward, _terminal, training=self.training)
