...
class Graph():
    ...
    def node2vec_walk(self, walk_length, start_node):
        ...
        while len(walk) < walk_length:
            ...
            num_of_edges_per_node = list(map(lambda x: len(G[cur][x].values()), G.neighbors(cur)))
            cur_nbrs = np.repeat(sorted(G.neighbors(cur)), num_of_edges_per_node)
...
def learn_embeddings(walks, dimensions, window_size, iters, output):
    ...
    model = Word2Vec(
        walks, size=dimensions, window=window_size, min_count=0, sg=1,
        workers=multiprocessing.cpu_count(), iter=iters
    )
    model.wv.save_word2vec_format(output)

    return

def get_embeddings(dot_string, is_directed, is_weighted, p, q,
                   num_walks, walk_length, dimensions, window_size,
                   iters, output, log):
    '''
